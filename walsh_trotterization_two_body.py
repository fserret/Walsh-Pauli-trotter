...
from walsh_pauli_decomposition import decompose_potential_term, decompose_laplacian_term, decompose_interaction_term
...
def create_trotter_step_circuit(N, dt, V0=1.0,kappa=1, interpolate=False, MCP_interaction=False, interaction=True):
...
def simulate_quantum_time_evolution(N, T_final=1.0, n_steps=50, V0=0.0, initial_state="gaussian", use_quantum_encoding=True,interpolate=False, interaction=True):
    ...
    trotter_circuit = create_trotter_step_circuit(N, dt, V0,interpolate=interpolate,interaction=interaction)
...
if __name__ == "__main__":
    ...
    times_wp, psi_wp, prob_wp = simulate_quantum_time_evolution(
            ...
            use_quantum_encoding=True, interpolate=interpolate, interaction=interaction
        )

    x_values = np.linspace(0, 1, 2**N, endpoint=False)
    plt.contourf(x_values,x_values,prob_wp[0].reshape((2**N,2**N)))
    plt.show()

    plt.contourf(x_values,x_values,prob_wp[40].reshape((2**N,2**N)))
    plt.show()

    plt.contourf(x_values,x_values,prob_wp[60].reshape((2**N,2**N)))
    plt.show()

    plt.contourf(x_values,x_values,prob_wp[80].reshape((2**N,2**N)))
    plt.show()

    plt.contourf(x_values,x_values,prob_wp[100].reshape((2**N,2**N)))
    plt.show()
